...
def test_roundtrip():
    ...
    for oid in ("TC-Z03-F02-LIDAR-OBS001", "TC-Z03-F02-IMG-OBS002", "TC-Z03-F02-DRONE-OBS003"):
        cur_clean.execute("DELETE FROM provenance WHERE measurement_id IN (SELECT measurement_id FROM measurement WHERE observation_id = ?) OR raw_artifact_id IN (SELECT artifact_id FROM raw_artifact WHERE observation_id = ?)", (oid, oid))
        cur_clean.execute("DELETE FROM measurement WHERE observation_id = ?", (oid,))
        cur_clean.execute("DELETE FROM raw_artifact WHERE observation_id = ?", (oid,))
        cur_clean.execute("DELETE FROM field_observation_meta WHERE observation_id = ?", (oid,))
        cur_clean.execute("DELETE FROM observation WHERE observation_id = ?", (oid,))
    conn.commit()
...
>>>>>>> 0f4d5d6 (feat: integrate frozen Group 3 observation schema v2.2)
        field_meta={
