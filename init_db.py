...
def run_sqlite(url):
    ...
    if migration_file.name in applied:
        ...
        conn.execute(
            ...
            ("0.2", now(), "Adds field_observation_meta, raw_artifact, capture_method, observed_at, geography.scope"),
        )
        conn.commit()
        print(f"[init_db] Applied {migration_file.name} — VANA schema v0.2 ready.")
